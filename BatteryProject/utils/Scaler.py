...
class Scaler_nasa():
    def __init__(self,data): #  data.shape (N,L,C)
        self.data = data
...
class Scaler_all():
    ...
    def standerd(self, data):
        X = (data - self.mean) / (self.var + 1e-6)
        return X

    def minmax(self,data, feature_range=(0,1)):
        if feature_range == (0,1):
            X = (data - self.min) / ((self.max - self.min) + 1e-6)
        elif feature_range == (-1,1):
            X = 2*(data - self.min) / ((self.max - self.min) + 1e-6)-1
        else:
            raise ValueError('feature_range error!')
        return X